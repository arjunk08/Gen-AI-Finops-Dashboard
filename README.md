...
- **Lightweight Cost Forecasting**: Implemented a **Daily Burn Rate Extrapolation** model using native Pandas arithmetic for future cost and token prediction in the dashboard. We intentionally removed heavy machine learning libraries like `Scikit-Learn` (Random Forest, Linear Regression). While Scikit-Learn is highly capable, its massive dependency footprint causes out-of-memory (OOM) crashes on constrained environments like Render's free tier. Furthermore, because cloud provider bills (like Vertex AI) arrive on highly irregular dates with erratic spikes, standard ML algorithms often overfit or fail to capture the true underlying consumption rate. By calculating an exact cumulative daily burn rate across the invoice's lifespan, we achieve highly accurate, mathematically stable 30-day projections while keeping the app's deployment weight virtually zero.
